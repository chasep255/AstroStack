...
star_pairs_t* matcher_match_pairs(matcher_t* matcher, stars_t* stars);
...
image_t* image_new_ppm(FILE* in)
{
	assert(in);
	
	char mn[3] = {0};
	if(fscanf(in, "%2s", mn) != 1)
		return NULL;
	...
	size_t w, h, max;
	if(fscanf(in, "%zu %zu %zu", &w, &h, &max) != 3)
		return NULL;
	
	if(!(w > 0 && h > 0 && max <= 0xffffL && max > 0))
		return NULL;
	...
	if(max > 255)
	{
		...
		{
			if(fread(buffer, 2, 3, in) != 3)
			{
				image_free(img);
				return NULL;
			}
			
			img->px[i].red = ((quantum_t)buffer[0] << 8) + buffer[1];
			img->px[i].green  = ((quantum_t)buffer[2] << 8) + buffer[3];
			img->px[i].blue  = ((quantum_t)buffer[4] << 8) + buffer[5];
		}
	}
	else
	{
		...
		for(size_t i = 0; i < (w * h); i++)
		{
			if(fread(buffer, 1, 3, in) != 3)
			{
				image_free(img);
				return NULL;
			}
			
			img->px[i].red  = (quantum_t)buffer[0] << 8;
			img->px[i].green  = (quantum_t)buffer[1] << 8;
			img->px[i].blue  = (quantum_t)buffer[2] << 8;
		}
	}
...
void stack_assert(bool condition, char* message)
{
	if(!condition)
	{
		if(message == NULL)
			message = "Terminating";
		fprintf(stderr, "STACK ASSERTION FAILURE: %s", message);
		exit(1);
	}
}
...
{	
	char cmd[4096];
	sprintf(cmd, "dcraw -4 -c -a -q 3 %s", file);
	FILE* f = popen(cmd, "r");
	if(!f)
	{
		char msg[4096];
		sprintf(msg, "%s failed.", cmd);
		stack_assert(false, msg);
	}
...
int main(int argc, char** argv)
{
	...
	for(int i = 0; i <= groupn; i++)
	{
		...
		darks_count += groups[i]->lights->size;
		flats_count += groups[i]->lights->size;
		dark_flats_count += groups[i]->lights->size;
	}
	...
	for(int i = 0; i <= groupn; i++)
	{
		...
		if(i == 0)
		{
			...
			stars_t* stars = stars_map(img, 0.99);
		...
		for(size_t j = i == 0; j < g->lights->size; j++)
		{
			...
			star_pairs_t* pairs = matcher_match_pairs(matcher, stars);
			stack_assert(pairs, "Out of Memory");
			affine_t a = affine_align_pairs(pairs, 10);
			
			#pragma omp ordered
			{
				stack_printf("Stacking Lights %s\n\tstars=%d\tmatches=%d\tdx=%f\tdy=%f\tangle=%f\n", 
						g->lights->file_names[j], stars->size, pairs->size, a.c, a.f, acosf(a.a) * 180.0f / M_PI);
